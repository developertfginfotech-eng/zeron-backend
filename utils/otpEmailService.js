...
class OTPEmailService {
  ...
  async createOTPRecord({ operation, propertyData, propertyId, requestedBy }) {
    try {
      ...
      const cleanupResult = await SimpleOTP.updateMany(
        { 
          requestedBy, 
          operation, 
          status: 'pending' 
      ...
      console.log('Cleaned up existing OTPs:', cleanupResult.modifiedCount);
  ...
  async verifyOTP(userId, inputOTP, operation) {
    try {
      ...
      console.log('Found OTP record:', otpRecord ? {
        ...
        requestedBy: otpRecord.requestedBy,
      ...
      if (!otpRecord) {
        ...
        const allUserOtps = await SimpleOTP.find({ requestedBy: userId }).sort({ createdAt: -1 }).limit(5);
        console.log('All recent OTPs for this user:', allUserOtps.map(otp => ({
          _id: otp._id,
          operation: otp.operation,
          status: otp.status,
          requestedBy: otp.requestedBy,
          createdAt: otp.createdAt
        })));
        
        // Debug: Check if there are any pending OTPs for this operation
        const allOperationOtps = await SimpleOTP.find({ operation, status: 'pending' }).sort({ createdAt: -1 }).limit(5);
        console.log('All pending OTPs for operation:', allOperationOtps.map(otp => ({
          _id: otp._id,
          requestedBy: otp.requestedBy,
          operation: otp.operation,
          status: otp.status
      ...
      console.log('- Status:', otpRecord.status);
      ...
      const isValidOTP = otpRecord.isValid(inputOTP);
      console.log('OTP validation result:', isValidOTP);
      console.log('Direct comparison (otpRecord.otp === inputOTP):', otpRecord.otp === inputOTP);
      console.log('OTP types - DB:', typeof otpRecord.otp, 'Input:', typeof inputOTP);
      
      if (!isValidOTP) {
        const attemptsRemaining = 3 - otpRecord.attempts;
        console.log('❌ OTP validation failed. Attempts remaining:', attemptsRemaining);
      ...
      // Mark as used
      console.log('✅ OTP validation successful! Marking as used...');
